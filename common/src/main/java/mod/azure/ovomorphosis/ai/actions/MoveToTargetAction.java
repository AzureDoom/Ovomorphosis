...
public final class MoveToTargetAction<E extends Mob> implements Action<E> {
     ...
     * {@link PlanFailureReason#FAILED_STUCK} up to GOAP via {@link AiKeys#LAST_PLAN_FEEDBACK}, instead of retrying
     * local recovery (detours, block breaks, jumps) forever. Local recovery attempts do not reset this counter — only
     * actual distance-to-target improvement does — so a mob that keeps detouring/jumping/breaking without ever closing
     * the distance will still terminate and let the planner pick a different goal.
     */
