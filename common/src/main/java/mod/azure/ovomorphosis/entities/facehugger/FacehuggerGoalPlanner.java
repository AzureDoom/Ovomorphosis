...
public final class FacehuggerGoalPlanner implements GoalPlanner<FacehuggerEntity, AiGoalType> {
    ...
    public PlannedGoal<FacehuggerEntity, AiGoalType> chooseGoal(
        ...
        if (activeGoalType != null) {
            if (activeGoalType.equals(AiGoalType.INFECT_HOST)) {
                infectScore += HYSTERESIS_BONUS;
            } else if (activeGoalType.equals(AiGoalType.STALK_HOST)) {
                stalkScore += HYSTERESIS_BONUS;
            } else if (activeGoalType.equals(AiGoalType.RETREAT_AND_HIDE)) {
