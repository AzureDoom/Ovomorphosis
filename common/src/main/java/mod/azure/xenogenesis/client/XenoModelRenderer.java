package mod.azure.xenogenesis.client;
...
public class XenoModelRenderer<T extends LivingEntity> extends AzEntityModelRenderer<T> {
    ...
    private static void applyClimbingRotations(
        ...
        var forward = lerpVec(
            wallCrawler.xenogenesis$getOldCrawlForward(),
            wallCrawler.xenogenesis$getCrawlForward(),
            partialTick
        ).scale(-1.0D);

        var up = lerpVec(
            wallCrawler.xenogenesis$getOldCrawlUp(),
            wallCrawler.xenogenesis$getCrawlUp(),
            partialTick
        );
        ...
        var distToBlock = Mth.lerp(
            partialTick,
            (float) wallCrawler.xenogenesis$getOldCrawlDistFromBlock(),
            (float) wallCrawler.xenogenesis$getCrawlDistFromBlock()
        );
